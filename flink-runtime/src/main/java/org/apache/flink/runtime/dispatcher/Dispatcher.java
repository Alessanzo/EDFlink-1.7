...
public abstract class Dispatcher extends FencedRpcEndpoint<DispatcherId> implements
	 ...
	 * EFLINK CODE - RESCALE SINGLE OPERATOR
	 -
	 */
	public CompletableFuture<Acknowledge> rescaleSingleOperator(JobID jobId,Map<String, Integer> operators,
																RescalingBehaviour rescalingBehaviour, Time timeout) {
		final CompletableFuture<JobMasterGateway> jobMasterGatewayFuture = getJobMasterGatewayFuture(jobId);

		return jobMasterGatewayFuture.thenCompose(
			(JobMasterGateway jobMasterGateway) ->
				jobMasterGateway.rescaleOperators(operators, rescalingBehaviour, timeout));
	}
